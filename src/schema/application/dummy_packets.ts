...
const config = (table) => ({
  ...
  ...getConfigDefaults.defaultIndexes('dummy_packets', table),
  // dummy_packets_total_length_idx: index('dummy_packets_total_length_idx').on(
  //   table.total_length,
  // ),
  ...Object.keys(fields).reduce((acc, field) => {
    const index_name = `dummy_packets_${field}_idx`;
    return {
      ...acc,
