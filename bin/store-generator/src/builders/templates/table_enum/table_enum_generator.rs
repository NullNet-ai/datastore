...
pub fn generate_table_enum(schema_path: &str, output_path: &str) -> io::Result<()> {
    ...
    writeln!(file, ") -> Result<String, DieselError> {{")?;
    writeln!(file, "    crate::utils::code_generator::generate_code(table, prefix_param, default_code_param).await")?;
    writeln!(file, "}}")?;
...
pub fn run_generator() -> io::Result<()> {
    ...
    // Default paths
    let schema_path = SCHEMA_FILE.as_str();
    let output_path = TABLE_ENUM_FILE.as_str();
