...
async fn init_counters_then_get_next_code_success() {
    ...
    let code1 = redis_code::get_next_code(&pool, &db, entity)
        .await
        .expect("get_code 1");
    let code2 = redis_code::get_next_code(&pool, &db, entity)
        .await
        .expect("get_code 2");
...
async fn init_counters_with_digits_format() {
    ...
    redis_code::init_counters(&pool, &db, &[(entity.to_string(), "DV".to_string(), 0, 6)])
        .await
        .expect("init");
    let code = redis_code::get_next_code(&pool, &db, entity)
        .await
        .expect("get_code");
...
async fn init_counters_empty_list_succeeds() {
    let pool = make_pool().await;
    let db = unique_db();
    redis_code::init_counters(&pool, &db, &[])
        .await
        .expect("init empty");
}
...
async fn init_counters_multiple_entities() {
    ...
    redis_code::init_counters(&pool, &db, &entities)
        .await
        .expect("init");
    let c1 = redis_code::get_next_code(&pool, &db, "e1")
        .await
        .expect("e1");
    let c2 = redis_code::get_next_code(&pool, &db, "e2")
        .await
        .expect("e2");
    assert_eq!(c1, "A11");
    assert_eq!(c2, "B01");
