...
pub fn generate_table_enum(schema_path: &str, output_path: &str) -> io::Result<()> {
    ...
    for table in &tables {
        ...
        writeln!(file, "use crate::models::{}::{}Model;", model_name, singular_name.to_case(Case::Pascal))?;
    }
    ...
    for table in &tables {
        ...
        writeln!(file, "            \"{}\" => Some(Table::{}),", snake_name, table.name.to_case(Case::Pascal))?;
    }
    writeln!(file, "            // Add other tables here")?;
    writeln!(file, "            _ => None,")?;
    writeln!(file, "        }}")?;
    ...
    writeln!(file, "        if !pluck_fields.is_empty() && record_value.is_object() {{")?;
    writeln!(file, "            let obj = record_value.as_object().unwrap();")?;
    writeln!(file, "            let mut filtered = Map::new();")?;
    writeln!(file, "")?;
    writeln!(file, "            for field in pluck_fields {{")?;
    writeln!(file, "                if let Some(val) = obj.get(&field) {{")?;
    writeln!(file, "                    filtered.insert(field, val.clone());")?;
    writeln!(file, "                }}")?;
    writeln!(file, "            }}")?;
    writeln!(file, "")?;
    writeln!(file, "            Value::Object(filtered)")?;
    writeln!(file, "        }} else {{")?;
    writeln!(file, "            record_value.clone() // fallback: return original value")?;
    writeln!(file, "        }}")?;
    writeln!(file, "    }}")?;
    writeln!(file, "")?;
    ...
    writeln!(file, "        generate_hypertable_timestamp_match!(self, conn, id, {})", table_list)?;
    writeln!(file, "    }}")?;
    writeln!(file, "")?;
    ...
    let table_model_list = tables.iter()
        .map(|t| {
    ...
    writeln!(file, "            {} // Add other tables and their models here as needed", table_model_list)?;
    writeln!(file, "        )")?;
    writeln!(file, "    }}")?;
    writeln!(file, "")?;
    ...
    writeln!(file, "        record: Value,")?;
    writeln!(file, "    ) -> Result<(), DieselError> {{")?;
    writeln!(file, "        generate_upsert_record_with_timestamp_match!(")?;
    writeln!(file, "            self,")?;
    writeln!(file, "            conn,")?;
    writeln!(file, "            record,")?;
    writeln!(file, "            {} // Add other tables and their models here as needed", table_model_list)?;
    ...
    writeln!(file, "        let result = diesel::insert_into(schema::counters::dsl::counters::table())")?;
    writeln!(file, "        .values(&new_counter)")?;
    writeln!(file, "            .on_conflict(schema::counters::entity)")?;
    writeln!(file, "            .do_update()")?;
    writeln!(file, "            .set(schema::counters::counter.eq(schema::counters::counter + 1))")?;
    writeln!(file, "            .returning((schema::counters::prefix, schema::counters::default_code, schema::counters::counter))")?;
    writeln!(file, "            .get_result::<(String, i32, i32)>(&mut conn).await")?;
    writeln!(file, "            .map_err(|e| {{")?;
    writeln!(file, "                log::error!(\"Error generating code: {{}}\", e);")?;
    writeln!(file, "                e")?;
    writeln!(file, "            }})?;")?;
    writeln!(file, "        ")?;
    writeln!(file, "        // Format the code")?;
    writeln!(file, "        let (prefix_val, default_code_val, counter_val) = result;")?;
