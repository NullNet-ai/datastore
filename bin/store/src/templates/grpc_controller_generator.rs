...
pub fn generate_grpc_controller(proto_path: &str, output_path: &str) -> io::Result<()> {
    ...
    let rpc_regex = Regex::new(r"rpc\s+(\w+)\s*\(\s*(\w+)\s*\)\s*returns\s*\(\s*(\w+)\s*\)").unwrap();
    ...
    if let Some(captures) = service_regex.captures(&proto_content) {
        ...
        return Err(io::Error::new(io::ErrorKind::InvalidData, "No service definition found in proto file"));
    }
    ...
    writeln!(file, "use crate::generated::store::store_service_server::{{StoreService, StoreServiceServer}};")?;
    writeln!(file, "use actix_web::{{HttpResponse, Responder, web}};")?;
    writeln!(file, "use serde::Serialize;")?;
    writeln!(file, "use std::net::SocketAddr;")?;
    writeln!(file, "use tonic::{{Request, Response, Status, transport::Server}};")?;
    ...
    writeln!(file, "    pub async fn init(addr: &str) -> Result<(), Box<dyn std::error::Error>> {{")?;
    writeln!(file, "        let addr: SocketAddr = addr.parse()?;  // Specify the type here")?;
    writeln!(file, "        let grpc_controller = GrpcController::new();")?;
    writeln!(file, "\n        println!(\"gRPC Server listening on {{}}\", addr);")?;
    writeln!(file, "\n        Server::builder()")?;
    writeln!(file, "            .add_service(")?;
    writeln!(file, "                {}Server::new(grpc_controller)", service_name)?;
    writeln!(file, "                    .max_decoding_message_size(50 * 1024 * 1024),")?;
    writeln!(file, "            )")?;
    ...
    for (method_name, request_type, response_type) in rpc_methods {
        ...
        writeln!(file, "    async fn {}(&self, request: Request<{}>) -> Result<Response<{}>, Status> {{", 
    ...
    writeln!(file, "    cfg.service(web::resource(\"/api/grpc/status\").route(web::get().to(grpc_status)));")?;
    writeln!(file, "}}")?;
