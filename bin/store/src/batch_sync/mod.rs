...
impl BatchSyncService {
            ...
            loop {
                ...
                if let Some(message) = Self::get_next_message(&queue).await {
                    if Self::process_insert_message(message.clone()).await {
                        ...
                        log::debug!("Successfully processed insert message for table: {}", message.table);
                    } else {
                        // Processing failed, put it back in the queue for retry
                        log::warn!("Failed to process insert message for table: {}, will retry", message.table);
                    ...
                    match rx.recv().await {
                        Some(message) => {
                            ...
                            if !Self::process_insert_message(message.clone()).await {
                                ...
                                log::warn!("Failed to process new insert message, adding to retry queue");
                                Self::add_to_queue(&queue, message).await;
                            } else {
                                log::debug!("Successfully processed new insert message for table: {}", message.table);
                            }
                        }
                        None => {
                            // Channel closed, which shouldn't happen in normal operation
                            log::error!("Insert message channel closed, terminating background processor");
                            break;
                        }
                    }
                }
            ...
            loop {
                ...
                if let Some(message) = Self::get_next_message(&queue).await {
                    if Self::process_update_message(message.clone()).await {
                        // Message processed successfully, remove from queue
                        log::debug!("Successfully processed update message for table: {}", message.table);
                    } else {
                        // Processing failed, put it back in the queue for retry
                        log::warn!("Failed to process update message for table: {}, will retry", message.table);
                        Self::add_to_queue(&queue, message).await;
                    ...
                    match rx.recv().await {
                        Some(message) => {
                            ...
                            if !Self::process_update_message(message.clone()).await {
                                // If processing fails, add to queue for retry
                                log::warn!("Failed to process new update message, adding to retry queue");
                                Self::add_to_queue(&queue, message).await;
                            } else {
                                log::debug!("Successfully processed new update message for table: {}", message.table);
                            }
                        }
                        None => {
                            // Channel closed, which shouldn't happen in normal operation
                            log::error!("Update message channel closed, terminating background processor");
                            break;
                        }
                    }
                }
    ...
    async fn process_update_message(message: SyncMessage) -> bool {
        ...
        let id = match message.record.get("id").and_then(|v| v.as_str()) {
            ...
            None => {
                log::error!("Error processing update message: ID field not found or not a string in record");
    ...
    pub async fn send_insert_message(table: String, record: Value) -> Result<(), String> {
        let sender = Self::get_insert_sender().ok_or_else(|| "Queue service not initialized".to_string())?;
        ...
        sender.send(message).await.map_err(|e| format!("Failed to send insert message: {}", e))
    }
    
    // Send an update message to the queue
    pub async fn send_update_message(table: String, record: Value) -> Result<(), String> {
        let sender = Self::get_update_sender().ok_or_else(|| "Queue service not initialized".to_string())?;
        ...
        sender.send(message).await.map_err(|e| format!("Failed to send update message: {}", e))
    }
