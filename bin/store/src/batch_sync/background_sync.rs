...
impl BackgroundSyncService  {
    pub async fn new() -> Result<Self, String> {
        ...
        let batch_sync_size = env::var("BATCH_SYNC_SIZE")
            ...
            .parse::<usize>()
            .unwrap_or(100);
        let batch_sync_enabled = env::var("BATCH_SYNC_ENABLED").unwrap_or_else(|_| "false".to_string()) == "true";
        let batch_sync_type = env::var("BATCH_SYNC_TYPE").unwrap_or_else(|_| "round-robin".to_string());
    ...
    fn row_to_value(&self, row: &tokio_postgres::Row) -> Value {
        ...
        for i in 0..row.len() {
            ...
            // Handle null values
            if row.try_get::<_, Option<String>>(i).unwrap_or(None).is_none() {
