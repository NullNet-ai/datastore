...
define_table_schema! {
    ...
    indexes: {
        ...
        system_indexes!("connections"),
        // Custom table-specific indexes
        idx_connections_device_id: {
            columns: ["device_id"],
            unique: false,
            type: "btree"
        },
        idx_connections_device_id_source_ip: {
            columns: ["device_id", "source_ip"],
            unique: false,
            type: "btree"
        },
    },
    foreign_keys: {
        ...
        system_foreign_keys!("connections"),
        fk_connections_device_id: { columns: ["device_id"], foreign_table: "devices", foreign_columns: ["id"], on_delete: "no action", on_update: "no action" },
    }
}
