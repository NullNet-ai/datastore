...
impl InitialEntityDataInitializer {
    ...
    pub async fn initialize(&self, _params: Option<InitializerParams>) -> Result<(), ApiError> {
        ...
        for (table_name, data_records) in entity_data {
            match self.process_entity_safe(table_name, data_records).await {
                ...
                Err(e) => {
                    log::error!("Failed to process entity {}: {}. Continuing with next entity.", table_name, e);
        ...
        log::info!(
            ...
            successful_entities, total_entities
        );
    ...
    async fn process_entity_safe(&self, table_name: &str, data_records: Vec<Value>) -> Result<(), ApiError> {
        // Wrap entity processing in comprehensive error handling
        match self.process_entity(table_name, data_records).await {
            Ok(_) => Ok(()),
            Err(e) => {
                log::error!("Entity processing failed for {}: {}. This will not crash the application.", table_name, e);
                Err(e)
            }
        }
    }
    
    async fn process_entity(&self, table_name: &str, data_records: Vec<Value>) -> Result<(), ApiError> {
        log::info!("Processing entity: {} with {} records", table_name, data_records.len());
        ...
        for (index, record) in data_records.iter().enumerate() {
            ...
            match self.process_record(table_name, record).await {
                Ok(_) => {
                    ...
                    log::debug!("Successfully processed record {} for table {}", index, table_name);
                }
                Err(e) => {
        ...
        log::info!(
            ...
            table_name, successful_records, total_records
        );
    ...
    async fn process_record(&self, table_name: &str, record: &Value) -> Result<(), ApiError> {
        ...
        match result {
            ...
            Err(e) => {
                ...
                log::error!("Error processing record for table {}: {}. Continuing with next record.", table_name, e);
    ...
    async fn process_record_internal(&self, table_name: &str, record: &Value) -> Result<(), ApiError> {
        // Clone the record so we can modify it
        let mut record_clone = record.clone();
        
        // Extract the record ID for existence check
        let record_id = record_clone.get("id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                ApiError::new(
                    StatusCode::BAD_REQUEST,
        ...
        if existing_record.is_some() {
            log::info!("Record with ID '{}' already exists in table '{}', skipping insertion", record_id, table_name);
            return Ok(());
        }
        ...
        if let Some(obj) = record_clone.as_object_mut() {
            ...
            log::debug!("Generated and assigned code to record for table {}", table_name);
        }
    ...
    fn validate_record_structure(&self, table_name: &str, record: &Value) -> Result<(), ApiError> {
        if let Some(obj) = record.as_object() {
            ...
            if let Some(valid_fields) = get_table_fields(table_name) {
                ...
                log::warn!("Could not retrieve field list for table '{}', skipping field validation", table_name);
            }
            ...
            if !obj.contains_key("id") {
                return Err(ApiError::new(
                    StatusCode::BAD_REQUEST,
                    format!("Record for table '{}' missing required 'id' field", table_name),
                ));
            }
            ...
            Err(ApiError::new(
                StatusCode::BAD_REQUEST,
                format!("Invalid record format for table '{}': expected JSON object", table_name),
            ))
        }
    }
}
...
mod tests {
    ...
    async fn test_error_resilient_initialization() {
        ...
        assert!(result.is_ok(), "Initialization should always return Ok to prevent crashes");
        ...
        assert!(!sample_connections.is_empty(), "Connections data should not be empty");
        assert!(!sample_packets.is_empty(), "Packets data should not be empty");
        ...
        assert!(first_connection.get("id").is_some(), "Connection record should have an id field");
        assert!(first_connection.get("created_by").is_some(), "Connection record should have a created_by field");
        assert!(first_connection.get("created_date").is_some(), "Connection record should have a created_date field");
        assert!(first_connection.get("created_time").is_some(), "Connection record should have a created_time field");
        assert!(first_connection.get("timestamp").is_some(), "Connection record should have a timestamp field");
        assert!(first_connection.get("hypertable_timestamp").is_some(), "Connection record should have a hypertable_timestamp field");
        assert!(first_connection.get("organization_id").is_some(), "Connection record should have an organization_id field");
        
        // Verify organization_id matches environment variable
        let conn_org_id = first_connection.get("organization_id").unwrap().as_str().unwrap();
        assert_eq!(conn_org_id, expected_org_id, "Connection organization_id should match DEFAULT_ORGANIZATION_ID");
        
        // Verify timestamp format (ISO 8601 with microseconds)
        let conn_timestamp = first_connection.get("timestamp").unwrap().as_str().unwrap();
        let conn_hypertable_timestamp = first_connection.get("hypertable_timestamp").unwrap().as_str().unwrap();
        assert!(conn_timestamp.contains("T"), "Timestamp should be in ISO 8601 format");
        assert!(conn_timestamp.ends_with("+00:00"), "Timestamp should have UTC timezone");
        assert_eq!(conn_timestamp, conn_hypertable_timestamp, "Timestamp and hypertable_timestamp should match");
        
        let first_packet = &sample_packets[0];
        assert!(first_packet.get("id").is_some(), "Packet record should have an id field");
        assert!(first_packet.get("created_by").is_some(), "Packet record should have a created_by field");
        assert!(first_packet.get("created_date").is_some(), "Packet record should have a created_date field");
        assert!(first_packet.get("created_time").is_some(), "Packet record should have a created_time field");
        assert!(first_packet.get("timestamp").is_some(), "Packet record should have a timestamp field");
        assert!(first_packet.get("hypertable_timestamp").is_some(), "Packet record should have a hypertable_timestamp field");
        assert!(first_packet.get("organization_id").is_some(), "Packet record should have an organization_id field");
        
        // Verify organization_id matches environment variable
        let packet_org_id = first_packet.get("organization_id").unwrap().as_str().unwrap();
        assert_eq!(packet_org_id, expected_org_id, "Packet organization_id should match DEFAULT_ORGANIZATION_ID");
        
        // Verify timestamp format (ISO 8601 with microseconds)
        let packet_timestamp = first_packet.get("timestamp").unwrap().as_str().unwrap();
        let packet_hypertable_timestamp = first_packet.get("hypertable_timestamp").unwrap().as_str().unwrap();
        assert!(packet_timestamp.contains("T"), "Timestamp should be in ISO 8601 format");
        assert!(packet_timestamp.ends_with("+00:00"), "Timestamp should have UTC timezone");
        assert_eq!(packet_timestamp, packet_hypertable_timestamp, "Timestamp and hypertable_timestamp should match");
        ...
        assert!(created_date.len() == 10, "Date should be in YYYY-MM-DD format");
