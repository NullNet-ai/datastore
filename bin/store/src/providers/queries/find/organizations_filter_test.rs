...
mod tests {
    ...
    fn test_organizations_filter_sql_generation() {
        ...
        let json_content =
            fs::read_to_string(json_path).expect("Failed to read organizations_filter.json");

        println!("  ✓ Loaded JSON file");

        // Parse the JSON into GetByFilter struct
        let filter: GetByFilter =
            serde_json::from_str(&json_content).expect("Failed to parse JSON into GetByFilter");
        ...
        assert!(
            ...
            "SQL should contain FROM organizations"
        );
        ...
        assert!(
            sql.contains("\"organizations\".\"id\""),
            "SQL should select organizations.id"
        );
        assert!(
            sql.contains("\"organizations\".\"code\""),
            "SQL should select organizations.code"
        );
        assert!(
            sql.contains("\"organizations\".\"name\""),
            "SQL should select organizations.name"
        );
        ...
        assert!(
            ...
            "SQL should join with account_organizations"
        );
        ...
        assert!(
            ...
            "SQL should contain categories filter"
        );
    ...
    fn test_organizations_filter_json_structure() {
        ...
        assert!(
            json_value.get("pluck").is_some(),
            "JSON should have 'pluck' field"
        );
        assert!(
            json_value.get("pluck_object").is_some(),
            "JSON should have 'pluck_object' field"
        );
        assert!(
            json_value.get("advance_filters").is_some(),
            "JSON should have 'advance_filters' field"
        );
        assert!(
            json_value.get("joins").is_some(),
            "JSON should have 'joins' field"
        );
        assert!(
            json_value.get("limit").is_some(),
            "JSON should have 'limit' field"
        );
        assert!(
            json_value.get("order_by").is_some(),
            "JSON should have 'order_by' field"
        );
        assert!(
            json_value.get("concatenate_fields").is_some(),
            "JSON should have 'concatenate_fields' field"
        );

        // Validate pluck array
        let pluck = json_value
            .get("pluck")
            .unwrap()
            .as_array()
            .expect("pluck should be an array");
        assert!(!pluck.is_empty(), "pluck array should not be empty");
        assert!(
            pluck.contains(&serde_json::json!("id")),
            "pluck should contain 'id'"
        );
        assert!(
            pluck.contains(&serde_json::json!("name")),
            "pluck should contain 'name'"
        );

        // Validate advance_filters
        let advance_filters = json_value
            .get("advance_filters")
            .unwrap()
            .as_array()
            .expect("advance_filters should be an array");
        assert!(
            !advance_filters.is_empty(),
            "advance_filters should not be empty"
        );

        // Check for status filter
        let has_status_filter = advance_filters
            .iter()
            .any(|filter| filter.get("field").and_then(|f| f.as_str()) == Some("status"));
        assert!(
            has_status_filter,
            "advance_filters should contain status filter"
        );

        // Validate joins
        let joins = json_value
            .get("joins")
            .unwrap()
            .as_array()
        ...
        let concatenate_fields = json_value
            .get("concatenate_fields")
            .unwrap()
            .as_array()
            .expect("concatenate_fields should be an array");
        assert!(
            !concatenate_fields.is_empty(),
            "concatenate_fields should not be empty"
        );
    ...
    fn test_organizations_filter_with_different_timezones() {
        ...
        let sql_manila = constructor_manila
            .construct()
        ...
        assert!(
            sql_manila.contains("SELECT"),
            "Manila SQL should contain SELECT"
        );
    ...
    async fn test_organizations_filter_controller_results_structure() {
        ...
        let resp = get_by_filter(req.clone(), path, body)
            .await
            .respond_to(&req);
        assert_eq!(
            resp.status(),
            actix_web::http::StatusCode::OK,
            "Controller should return 200 OK"
        );
        ...
        assert!(
            value
                ...
                .and_then(|v| v.as_str())
                .unwrap_or("")
                .contains("Filter operation completed"),
            "Response message should indicate filter completion"
        );
        let data = value
            .get("data")
        ...
        assert_eq!(
            count as usize,
            data.len(),
            "Response count should equal data length"
        );

        if let Some(first) = data.get(0).and_then(|v| v.as_object()) {
            assert!(
                first.get("organizations").is_some(),
                "Each record should contain aggregated 'organizations' array"
            );
            assert!(
                first.get("created_by").is_some(),
                "Each record should contain aggregated 'created_by' array"
            );
            assert!(
                first.get("updated_by").is_some(),
                "Each record should contain aggregated 'updated_by' array"
            );
            assert!(
                first.get("created_by_account_organizations").is_some(),
                "Each record should contain aggregated 'created_by_account_organizations' array"
            );
            assert!(
                first.get("updated_by_account_organizations").is_some(),
                "Each record should contain aggregated 'updated_by_account_organizations' array"
            );
            assert!(
                first.get("district_orgs").is_some(),
                "Each record should contain aggregated 'district_orgs' array"
            );
            assert!(
                first.get("district_superintendent").is_some(),
                "Each record should contain aggregated 'district_superintendent' array"
            );
            assert!(
                first.get("superintendent").is_some(),
                "Each record should contain aggregated 'superintendent' array"
            );
            assert!(
                first.get("principal").is_some(),
                "Each record should contain aggregated 'principal' array"
            );
        }
