...
pub async fn initialize() -> std::io::Result<(Client, String)> {
    ...
    let endpoint_url = std::env::var("STORAGE_ENDPOINT")
    ...
    println!("✅ S3 client configured successfully for endpoint: {}", endpoint_url);
    ...
    let existing_buckets = match buckets_result {
        ...
        Err(e) => {
            ...
            eprintln!("   2. Check network connectivity (try curl {})", endpoint_url);
            eprintln!("   3. Verify credentials: ACCESS_KEY={}, SECRET_KEY=***", 
    ...
    if existing_buckets.contains(&bucket_name) {
        println!("✅ Bucket '{}' already exists (found in bucket list)", bucket_name);
        ...
        .await
    {
        ...
        Err(e) => {
            println!("Bucket '{}' does not exist, will attempt to create...", bucket_name);
            println!("Head bucket error details: {}", e);
            
            // Log detailed error information for debugging
            if let Some(service_error) = e.as_service_error() {
                println!("Head bucket service error code: {:?}", service_error.code());
                println!("Head bucket service error message: {:?}", service_error.message());
            }
    ...
    match create_result {
        ...
        Err(e) => {
             ...
             if let Some(service_error) = e.as_service_error() {
                 ...
                 if service_error.is_bucket_already_exists() || service_error.is_bucket_already_owned_by_you() {
                 ...
                 if let Some(code) = service_error.code() {
                     match code {
                         ...
                         "InvalidBucketName" => {
                             eprintln!("💡 Invalid bucket name '{}' - check naming rules", bucket_name);
                         }
                         _ => {
                     ...
                     if let Ok(list_output) = client.list_buckets().send().await {
                         let current_buckets: Vec<String> = list_output.buckets()
                             .iter()
             ...
             eprintln!("   - Access Key: {}", std::env::var("STORAGE_ACCESS_KEY").unwrap_or_else(|_| "<not_set>".to_string()));
             eprintln!("   - Secret Key: {}", std::env::var("STORAGE_SECRET_KEY").unwrap_or_else(|_| "<not_set>".to_string()));
             ...
             if error_string.contains("dispatch failure") {
                 ...
                 eprintln!("   - For HTTPS endpoints, try setting STORAGE_DISABLE_SSL_VERIFICATION=true");
...
pub fn get_client_region(client: &Client) -> Option<String> {
    ...
    client.config().region().map(|region| region.as_ref().to_string())
}
