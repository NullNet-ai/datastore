...
mod redis_cache_tests {
    ...
    fn create_test_redis_cache_with_url(url: String) -> Option<RedisCache<String, TestValue>> {
        ...
        match RedisCache::new(url, Some(Duration::from_secs(60))) {
            ...
            Err(e) => {
                ...
                println!("Redis not available for testing: {} - this is expected in some environments", e);
                None
            }
        }
    }
    ...
    fn test_redis_cache_creation_failure() {
        ...
        let malformed_result = RedisCache::<String, String>::new(
        ...
        assert!(invalid_port_result.is_err(), "Should fail with invalid port");
    }
    ...
    fn test_redis_basic_crud_operations() {
        ...
        let key = format!("test_key_{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos());
        let value = TestValue {
            id: 1,
        ...
        for attempt in 0..10 {
            match cache.get(&key) {
                Some(val) => {
                    retrieved = Some(val);
                    println!("Successfully retrieved key {} on attempt {}", key, attempt + 1);
                    break;
                }
                None => {
        ...
        if retrieved.is_none() {
            println!("Failed to retrieve key after 10 attempts. Last error: {:?}", last_error);
            println!("Cache length: {}", cache.len());
            println!("Cache contains key: {}", cache.contains_key(&key));
        }
        
        assert_eq!(retrieved, Some(value.clone()), "Should retrieve the inserted value");
        ...
        // Verify removal
        assert!(!cache.contains_key(&key), "Cache should not contain key after removal");
    ...
    fn test_redis_clear_operation() {
        ...
        println!("Successfully inserted and verified {} out of 5 items", successfully_inserted);
        ...
        for attempt in 0..5 {
            ...
            println!("Clear attempt {}: Cache still has {} items", attempt + 1, cache.len());
            std::thread::sleep(Duration::from_millis(200));
        }
    ...
    fn test_redis_concurrent_operations() {
        ...
        for i in 0..10 {
            ...
            let handle = thread::spawn(move || {
                let key = format!("concurrent_thread_{}_{}_{}", i, std::process::id(), std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos());
                let value = TestValue {
                    id: i,
                ...
                // Verify removal
                assert!(!cache_clone.contains_key(&key), "Key should be removed: {}", key);
            });
            handles.push(handle);
        }
    ...
    fn test_redis_connection_scenarios() {
        ...
        let valid_result = RedisCache::<String, String>::new(
            "redis://127.0.0.1:6379".to_string(),
            None
        );
        ...
        let malformed_url_result = RedisCache::<String, String>::new(
            "this-is-not-a-url".to_string(),
            None
        );
        assert!(malformed_url_result.is_err(), "Should fail with malformed URL");
    ...
    fn test_redis_error_handling() {
        ...
        match create_test_redis_cache_with_db(12) {
            Some(cache) => {
                ...
                let test_key = format!("error_handling_key_{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos());
                ...
                for attempt in 0..5 {
                    ...
                    if result.is_some() {
                        println!("Successfully retrieved {} on attempt {}", test_key, attempt + 1);
                        break;
                    }
                    std::thread::sleep(Duration::from_millis(100));
                }
    ...
    fn test_graceful_redis_unavailability() {
        ...
        if !redis_available {
            ...
            assert!(cache.is_some(), "Should be able to create cache when Redis is available");
    ...
    fn test_network_edge_cases() {
        ...
        // Test with empty URL
        let empty_url_result = RedisCache::<String, String>::new(
            "".to_string(),
            None
        );
        ...
        let no_protocol_result = RedisCache::<String, String>::new(
        ...
        let no_port_result = RedisCache::<String, String>::new(
        ...
        let ipv6_result = RedisCache::<String, String>::new(
            "redis://[::1]:6379".to_string(),
            None
        );
        match ipv6_result {
        ...
        match auth_result {
            ...
            Err(e) => println!("✓ Authenticated connection failed (expected without proper auth): {}", e),
        }
    ...
    fn test_connection_recovery() {
        ...
        println!("Connection recovery test: {}/{} successful connections", successful_connections, total_attempts);
    ...
    fn test_basic_cache_operations() {
        ...
        assert!(key_exists, "Cache should contain key {} after insertion", key);
    ...
    fn test_cache_clear() {
        ...
        for i in 0..5 {
            ...
            for attempt in 0..5 {
                if cache.contains_key(&key) {
                    found = true;
                    println!("Successfully verified key {} on attempt {}", key, attempt + 1);
                    break;
                }
                std::thread::sleep(Duration::from_millis(100));
            }
    ...
    fn test_concurrent_operations() {
        ...
        for i in 0..10 {
            let cache_clone = Arc::clone(&cache);
            let handle = thread::spawn(move || {
                let key = format!("concurrent_key_{}_{}_{}", i, std::process::id(), std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos());
                let value = TestValue {
                    id: i,
                    name: format!("Concurrent Item {}", i),
                    data: vec![format!("concurrent_data_{}", i)],
    ...
    fn test_error_recovery() {
        // Use a different database to avoid interference with other tests
        let cache = match create_test_redis_cache_with_db(13) {
            Some(cache) => cache,
            None => {
                println!("Redis not available - skipping test");
                return;
        ...
        // Use unique key to avoid conflicts
        let test_id = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos();
        let key = format!("error_recovery_key_{}", test_id);
        let value = TestValue {
            id: 99,
            name: "Error Recovery Test".to_string(),
            data: vec!["recovery_data".to_string()],
        };
        ...
        println!("Re-inserting key: {}", key);
        cache.insert(key.clone(), value.clone());
        
        // Wait for Redis to process the re-insert
        std::thread::sleep(Duration::from_millis(300));
        
        // Retry logic for retrieval after re-insert
        let mut retrieved_after = None;
        for attempt in 0..10 {
            retrieved_after = cache.get(&key);
            if retrieved_after.is_some() {
                println!("Successfully re-retrieved key {} on attempt {}", key, attempt + 1);
                break;
            }
            println!("Re-insert attempt {}: Failed to retrieve key {}", attempt + 1, key);
            std::thread::sleep(Duration::from_millis(200));
        }
        assert_eq!(retrieved_after, Some(value));
...
mod integration_tests {
    ...
    fn test_redis_integration_with_real_data() {
        ...
        let test_data = vec![
            ("user:1", IntegrationTestValue { timestamp: 1234567890, message: "User login".to_string(), counter: 1 }),
            ("user:2", IntegrationTestValue { timestamp: 1234567891, message: "User logout".to_string(), counter: 2 }),
            ("session:abc123", IntegrationTestValue { timestamp: 1234567892, message: "Session created".to_string(), counter: 3 }),
        ];
        ...
        for (key, expected_value) in &test_data {
            ...
            assert_eq!(retrieved, Some(expected_value.clone()), "Failed to retrieve key: {}", key);
        }
        ...
        println!("Redis integration test completed successfully with {} items", test_data.len());
    }
}