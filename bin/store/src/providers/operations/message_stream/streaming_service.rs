                ...
                while let Some(message) = main_stream.emit_message().await {
                    ...
                    if let Some(org_id) = organization_id {
                        if let Some(_org_clients) =
                            crate::providers::operations::message_stream::gateway::get_organization_clients(&org_id)
                        {
            ...
            .await
        {
            ...
            if let Some(item) = sample_items.first() {
                if let Some(org_id) = item.content.get("organization_id").and_then(|v| v.as_str()) {
                    ...
                    let new_bucket =
                        crate::providers::operations::message_stream::token_bucket::TokenBucket::new_without_consumer(
                            channel_name,
                            bucket_capacity,
                        );

                    let actual_bucket: Arc<TokenBucket> = self
                        .shared_state
                    ...
                    if Arc::ptr_eq(&new_bucket, &actual_bucket) {
                        ...
                        crate::providers::operations::message_stream::gateway::add_channel_to_organization(
                            org_id,
                            channel_name,
                        );
                        info!(
