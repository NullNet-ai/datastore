...
impl TokenBucket {
    ...
    pub async fn receive_message(self: &Arc<Self>, msg: Message) -> bool {
        ...
        if *tokens > 0 {
            *tokens -= 1;
            ...
            debug!(
                "📥 RECEIVE_MESSAGE - Buffer size: {}, Tokens: {}, Bucket: {}",
    ...
    pub async fn emit_message(self: &Arc<Self>) -> Option<Message> {
        ...
        if let Some(ref message) = msg {
            ...
            *tokens = std::cmp::min(*tokens + 1, capacity);
            ...
            let message_id = if let Ok(parsed) = serde_json::from_value::<
            ...
            if was_backpressured
                || (buffer_empty && current_tokens > 0)
                || current_tokens == capacity
            {
                debug!(
                    "🔔 DRAIN NOTIFICATION - Reason: {}, Bucket: {}, Buffer: {}, Tokens: {}",
                    if was_backpressured {
    ...
    fn start_sequential_consumer(self: &Arc<Self>) {
        ...
        debug!(
            "🔧 START_SEQUENTIAL_CONSUMER CALLED - Bucket: {}",
            bucket.name
        );
        ...
        if should_start_consumer {
            ...
            tokio::spawn(async move {
                ...
                loop {
                    ...
                    info!(
                        "Consumer processed {} messages for bucket: {}",
                        message_count, bucket.name
                    );
                }
            });
        }
