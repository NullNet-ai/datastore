...
mod tests {
    ...
    async fn test_register_channel() {
        ...
        let result = state.register_channel(
            "test_channel",
            "org_123",
            token_bucket.clone(),
            10
        ...
        assert!(state.active_channels.lock().await.contains_key("test_channel"));
        
        let org_channels = state.organization_channels.lock().await;
        assert!(org_channels.get("org_123").unwrap().contains("test_channel"));
    ...
    async fn test_register_existing_channel() {
        ...
        let result1 = state.register_channel(
            "test_channel",
            "org_123",
            token_bucket1.clone(),
            10
        ...
        let result2 = state.register_channel(
            "test_channel",
            "org_123",
            token_bucket2.clone(),
            20
        ...
        assert_eq!(result1.get_high_watermark().await, result2.get_high_watermark().await);
    }
    ...
    async fn test_processing_queue() {
        ...
        assert_eq!(state.dequeue_for_processing().await, Some("channel1".to_string()));
        assert_eq!(state.dequeue_for_processing().await, Some("channel2".to_string()));
    ...
    async fn test_get_channel_organization() {
        ...
        assert_eq!(
            state.get_channel_organization("nonexistent").await,
            None
        );
    }
    ...
    async fn test_set_tokens_with_buffer_overflow() {
        ...
        for i in 0..4 {
            bucket.receive_message(create_simple_message(&format!("msg{}", i))).await;
        }
    ...
    async fn test_message_creation() {
        ...
        if let Ok(parsed) = serde_json::from_value::<serde_json::Map<String, serde_json::Value>>(message.0.clone()) {
            assert_eq!(parsed.get("id").and_then(|v| v.as_str()), Some("test_id"));
            assert_eq!(parsed.get("organization_id").and_then(|v| v.as_str()), Some("org_123"));
            assert_eq!(parsed.get("event_name").and_then(|v| v.as_str()), Some("test_event"));
        } else {
    ...
    async fn test_multiple_organizations_channels() {
        ...
        state.register_channel("channel2", "org1", bucket2, 10).await;
        ...
        assert_eq!(state.get_channel_organization("channel1").await, Some("org1".to_string()));
        assert_eq!(state.get_channel_organization("channel2").await, Some("org1".to_string()));
        assert_eq!(state.get_channel_organization("channel3").await, Some("org2".to_string()));
    }
}