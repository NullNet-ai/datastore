...
mod tests {
    use crate::{
        ...
        structs::core::{GetByFilter, FilterCriteria, FilterOperator},
    };
    use serde_json::json;
    use std::fs;
    use std::path::Path;
    ...
    async fn should_create_contacts_basic_fields_scenario() {
        ...
        let mut payload_filters = PayloadFilterScenarios::new(
            "contacts".to_string(),
            default_filter,
        );
        ...
        let result = payload_filters.create_scenario_with_description(
            ...
            "Retrieve records with id, first_name, and last_name fields from the contacts table".to_string(),
            scenario_filter.clone(),
        );

        println!("  ✓ Creating scenario with basic fields (id, first_name, last_name)");
        assert!(result.is_ok(), "Failed to create contacts_basic_fields scenario");
        assert!(payload_filters.has_scenario("contacts_basic_fields"));

        // Verify the scenario was created correctly
        let created_scenario = payload_filters.get_scenario("contacts_basic_fields").unwrap();
        ...
        assert!(true, "Test completed - contacts basic fields scenario creation");
    }
    ...
    async fn should_create_contacts_active_status_scenario() {
        ...
        assert!(result.is_ok(), "Failed to create contacts_active_status scenario");
        assert!(payload_filters.has_scenario("contacts_active_status"));

        // Verify the scenario was created correctly
        let created_scenario = payload_filters.get_scenario("contacts_active_status").unwrap();
        ...
        // Keep the scenario file for persistent use
        println!("  ✓ Scenario file persisted at: {:?}", file_path);
        assert!(true, "Test completed - contacts active status scenario creation");
    }
    ...
    async fn should_load_and_use_created_scenarios() {
        ...
        payload_filters.set_current_scenario("contacts_basic_fields").unwrap();
        ...
        payload_filters.set_current_scenario("contacts_active_status").unwrap();
        let current_filter = payload_filters.get_current_filter();
