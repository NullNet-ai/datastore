...
use crate::structs::structs::{FilterCriteria, FilterOperator, MatchPattern, Join, FieldRelation, RelationEndpoint, Aggregation, AggregationType, LogicalOperator, AggregationOrder};
...
pub fn convert_filter_criteria(proto_filter: &store::FilterCriteria) -> Option<FilterCriteria> {
    ...
    match &proto_filter.filter_type {
        Some(FilterType::Criteria(criteria)) => {
            ...
            let values: Vec<serde_json::Value> = criteria.values.iter()
...
pub fn convert_join(proto_join: &store::Join) -> Option<Join> {
    let field_relation = proto_join.field_relation.as_ref().map(|fr| {
        let to = fr.to.as_ref().map(|to_endpoint| RelationEndpoint {
        ...
        let from = fr.from.as_ref().map(|from_endpoint| RelationEndpoint {
