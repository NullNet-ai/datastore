...
impl MigrationGenerator {
    ...
    pub fn generate_migration(
        ...
        let migration_dir = format!("{}/{}_{}", MIGRATIONS_DIR, timestamp, migration_name);
...
mod tests {
    ...
    fn test_diesel_to_postgres_type() {
        ...
        assert_eq!(
            MigrationGenerator::diesel_to_postgres_type("Text", false).unwrap(),
            "TEXT NOT NULL"
        );
        assert_eq!(
            MigrationGenerator::diesel_to_postgres_type("Nullable<Text>", false).unwrap(),
            "TEXT NOT NULL"
        );
        assert_eq!(
            MigrationGenerator::diesel_to_postgres_type("Int4", false).unwrap(),
            "INTEGER NOT NULL"
        );

        // Test with migration_nullable = true (nullable)
        assert_eq!(
            MigrationGenerator::diesel_to_postgres_type("Text", true).unwrap(),
            "TEXT"
        );
        assert_eq!(
            MigrationGenerator::diesel_to_postgres_type("Nullable<Text>", true).unwrap(),
            "TEXT"
        );
        assert_eq!(
            MigrationGenerator::diesel_to_postgres_type("Nullable<Array<Text>>", true).unwrap(),
            "TEXT[]"
        );
        assert_eq!(
            MigrationGenerator::diesel_to_postgres_type("Nullable<Jsonb>", true).unwrap(),
            "JSONB"
        );
    }
    ...
    fn test_generate_create_index_sql() {
        let result = MigrationGenerator::generate_create_index_sql(
            "users",
            "idx_users_email",
            "\"email\"",
            None,
        ...
        let result_with_type = MigrationGenerator::generate_create_index_sql(
            "users",
            "idx_users_data",
            "\"data\"",
            Some("gin"),
    ...
    fn test_generate_add_column_sql() {
        let result =
            MigrationGenerator::generate_add_column_sql("users", "email", "Nullable<Text>", true);
        assert!(result.is_ok());
        let sql = result.unwrap();
