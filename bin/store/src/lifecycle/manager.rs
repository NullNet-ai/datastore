...
use super::state::{StateManager, LifecyclePhase, ComponentStatus};
use super::logging::{LifecycleLogger, LogConfig, LogLevel, LogCategory};
...
impl LifecycleManager {
    ...
    pub async fn execute(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        // Set initial phase
        self.state_manager.set_phase(LifecyclePhase::Initializing).await;
        ...
        match self.execute_startup().await {
            ...
            Err(e) => {
                ...
                self.state_manager.set_phase(LifecyclePhase::Error(error_msg.clone())).await;
                return Err(e);
            }
        }
        ...
        self.state_manager.set_phase(LifecyclePhase::ShuttingDown).await;
        match self.execute_shutdown().await {
            ...
            Err(e) => {
                ...
                self.state_manager.set_phase(LifecyclePhase::Error(error_msg)).await;
                return Err(e);
            }
        }
    ...
    async fn execute_startup(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        self.state_manager.register_component("StartupManager".to_string()).await;
        self.state_manager.register_component("RuntimeManager".to_string()).await;
        self.state_manager.register_component("ShutdownManager".to_string()).await;
        self.state_manager.register_component("DatabasePool".to_string()).await;
        self.state_manager.register_component("S3Client".to_string()).await;
    ...
    async fn execute_runtime(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let pool = self.startup_manager.pool.as_ref()
            .ok_or("Database pool not initialized")?;
        let s3_client = self.startup_manager.s3_client.as_ref()
            .ok_or("S3 client not initialized")?;
        let bucket_name = self.startup_manager.bucket_name.as_ref()
    ...
    async fn execute_shutdown(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        self.state_manager.update_component_status("StartupManager", ComponentStatus::Stopping).await;
        self.state_manager.update_component_status("RuntimeManager", ComponentStatus::Stopping).await;
        ...
        self.state_manager.update_component_status("StartupManager", ComponentStatus::Stopped).await;
        self.state_manager.update_component_status("RuntimeManager", ComponentStatus::Stopped).await;
        self.state_manager.update_component_status("ShutdownManager", ComponentStatus::Stopped).await;
