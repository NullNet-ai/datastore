    ...
    async fn shutdown(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        if let Some(tx) = self.shutdown_tx.take() {
            ...
            if let Err(e) = tx.send(()).await {
                warn!("[SHUTDOWN] Failed to send shutdown signal to {}: {}", self.name, e);
            } else {
...
impl ShutdownManager {
    ...
    pub fn with_config(config: ShutdownConfig) -> Self {
        info!("[SHUTDOWN] Initializing shutdown manager with config: {:?}", config);
    ...
    pub fn register_service(&mut self, service: Box<dyn ShutdownService + Send + Sync>) {
        info!("[SHUTDOWN] Registering service for shutdown: {}", service.name());
    ...
    async fn execute_shutdown_stages(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        *self.stage.write().await = ShutdownStage::CleanupResources;
        ...
        if elapsed > total_timeout {
            warn!("[SHUTDOWN] Shutdown took longer than expected: {:?} > {:?}", elapsed, total_timeout);
        }
            ...
            for mut service in http_services {
                if let Err(e) = service.shutdown().await {
                    ...
                    if let Err(fe) = service.force_shutdown().await {
                        error!("[SHUTDOWN] Force shutdown also failed for {}: {}", service.name(), fe);
                    }
                }
            }
        };
    ...
    async fn stop_background_services(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let force_future = async {
            for mut service in self.services.drain(..) {
                if let Err(e) = service.force_shutdown().await {
                    error!("[SHUTDOWN] Force shutdown failed for {}: {}", service.name(), e);
                }
            }
        };
