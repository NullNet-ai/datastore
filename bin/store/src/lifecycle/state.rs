...
impl StateManager {
    ...
    pub async fn set_phase(&self, phase: LifecyclePhase) {
        ...
        info!("[STATE] Lifecycle phase changed: {:?} -> {:?}", old_phase, phase);
    ...
    pub async fn register_component(&self, name: String) {
        ...
        self.components.write().await.insert(name.clone(), component_info);
    ...
    pub async fn update_component_status(&self, name: &str, status: ComponentStatus) {
        ...
        if let Some(event) = event {
            info!("[STATE] Component status changed: {} -> {:?}", name, event.new_status);
    ...
    pub async fn update_component_metadata(&self, name: &str, key: String, value: String) {
        ...
        if let Some(component) = components.get_mut(name) {
            ...
            debug!("[STATE] Component metadata updated: {} -> {}={}", name, key, value);
        } else {
            warn!("[STATE] Attempted to update metadata for unknown component: {}", name);
        }
    }
    ...
    pub async fn generate_health_report(&self) -> String {
        ...
        report.push_str(&format!("Processed Requests: {}\n", health_metrics.processed_requests));
    ...
    pub async fn start_monitoring(&self) {
        ...
        *monitoring_enabled.write().await = true;
        ...
        info!("[STATE] Background monitoring started with interval: {:?}", interval_duration);
    }
    ...
    async fn update_component_health_checks(components: &Arc<RwLock<HashMap<String, ComponentInfo>>>) {
    ...
    pub async fn set_metrics_interval(&mut self, interval: Duration) {
        ...
        info!("[STATE] Metrics collection interval updated to: {:?}", interval);
    }
