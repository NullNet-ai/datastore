...
use crate::providers::storage::cache::{CacheConfig, cache};
...
use crate::lifecycle::logging::{LogLevel, LogCategory};
use std::sync::Arc;
...
impl StartupManager {
    ...
    async fn validate_configuration(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        if !validation.is_valid {
            ...
            return Err(format!("Configuration validation failed with {} errors", validation.errors.len()).into());
        }
    ...
    fn validate_required_variables(&self, validation: &mut StartupValidation) {
        ...
        for var in required_vars {
            if std::env::var(var).is_err() {
                validation.errors.push(format!("Missing required environment variable: {}", var));
        ...
        for var in recommended_vars {
            if std::env::var(var).is_err() {
                validation.warnings.push(format!("Recommended environment variable not set: {} (using default)", var));
            }
        }
    }
    ...
    fn validate_port_configurations(&self, validation: &mut StartupValidation) {
        ...
        if let Ok(port_str) = std::env::var("PORT") {
            match port_str.parse::<u16>() {
                Ok(port) => {
                    if port < 1024 {
                        validation.warnings.push("PORT is set to a privileged port (<1024), ensure proper permissions".to_string());
                    }
                },
                Err(_) => {
                    validation.errors.push("PORT must be a valid integer".to_string());
                    validation.is_valid = false;
                }
            }
        }
        ...
        if let Ok(grpc_port_str) = std::env::var("GRPC_PORT") {
            match grpc_port_str.parse::<u16>() {
                Ok(port) => {
                    ...
                    if let Ok(http_port_str) = std::env::var("PORT") {
                        if let Ok(http_port) = http_port_str.parse::<u16>() {
                            if port == http_port {
                                validation.errors.push("GRPC_PORT cannot be the same as PORT".to_string());
                                validation.is_valid = false;
                            }
                        }
                    }
                ...
                Err(_) => {
                    validation.errors.push("GRPC_PORT must be a valid integer".to_string());
                    validation.is_valid = false;
                }
            }
        }
        ...
        if let Ok(socket_port_str) = std::env::var("SOCKET_PORT") {
            match socket_port_str.parse::<u16>() {
                ...
                Err(_) => {
                    validation.errors.push("SOCKET_PORT must be a valid integer".to_string());
                    validation.is_valid = false;
                }
            }
        }
    ...
    fn validate_database_configuration(&self, validation: &mut StartupValidation) {
        ...
        if let Ok(db_url) = std::env::var("DATABASE_URL") {
            ...
            if !db_url.contains("@") || !db_url.contains("/") {
                validation.errors.push("DATABASE_URL appears to be malformed (missing @ or / characters)".to_string());
                validation.is_valid = false;
            }
        }
        ...
        if let Ok(port_str) = std::env::var("POSTGRES_PORT") {
            match port_str.parse::<u16>() {
                Ok(port) => {
                    if port == 0 {
                        validation.errors.push("POSTGRES_PORT cannot be 0".to_string());
                        validation.is_valid = false;
                    }
                },
                Err(_) => {
                    validation.errors.push("POSTGRES_PORT must be a valid integer".to_string());
                    validation.is_valid = false;
                }
            }
        }
    ...
    fn validate_cache_configuration(&self, validation: &mut StartupValidation) {
        ...
        match cache_type.as_str() {
            "inmemory" | "redis" => {},
            _ => {
                validation.errors.push(format!("Invalid CACHE_TYPE '{}'. Must be 'inmemory' or 'redis'", cache_type));
                validation.is_valid = false;
            }
        }
        ...
        if cache_type == "redis" {
            if std::env::var("REDIS_CONNECTION").is_err() {
                validation.errors.push("REDIS_CONNECTION required when CACHE_TYPE is 'redis'".to_string());
                validation.is_valid = false;
            } else if let Ok(redis_url) = std::env::var("REDIS_CONNECTION") {
                if !redis_url.starts_with("redis://") {
                    validation.errors.push("REDIS_CONNECTION must be a valid Redis URL starting with 'redis://'".to_string());
                    validation.is_valid = false;
                }
            }
        }
        ...
        if let Ok(ttl_str) = std::env::var("CACHE_TTL") {
            match ttl_str.parse::<u64>() {
                Ok(ttl) => {
                    if ttl == 0 {
                        validation.warnings.push("CACHE_TTL is set to 0, cache entries will expire immediately".to_string());
                    }
                    if ttl > 86400 {
                ...
                Err(_) => {
                    validation.errors.push("CACHE_TTL must be a valid integer (seconds)".to_string());
                    validation.is_valid = false;
                }
            }
        }
    ...
    fn validate_storage_configuration(&self, validation: &mut StartupValidation) {
        ...
        if let Ok(disable_storage) = std::env::var("DISABLE_STORAGE") {
            match disable_storage.to_lowercase().as_str() {
                "true" | "false" => {},
                _ => {
                    validation.errors.push("DISABLE_STORAGE must be 'true' or 'false'".to_string());
                    validation.is_valid = false;
                }
            }
            ...
            if disable_storage.to_lowercase() != "true" {
                ...
                if let Ok(disable_ssl) = std::env::var("STORAGE_DISABLE_SSL_VERIFICATION") {
                    match disable_ssl.to_lowercase().as_str() {
                        "true" | "false" => {},
                        _ => {
                            validation.errors.push("STORAGE_DISABLE_SSL_VERIFICATION must be 'true' or 'false'".to_string());
                            validation.is_valid = false;
                        }
                    }
    ...
    fn validate_security_configuration(&self, validation: &mut StartupValidation) {
        ...
        if let Ok(jwt_secret) = std::env::var("JWT_SECRET") {
            if jwt_secret.len() < 32 {
                validation.errors.push("JWT_SECRET must be at least 32 characters long for security".to_string());
                validation.is_valid = false;
            }
            
            // Check for common weak secrets
            let weak_secrets = vec!["secret", "password", "123456", "admin", "test"];
            if weak_secrets.iter().any(|&weak| jwt_secret.to_lowercase().contains(weak)) {
                validation.warnings.push("JWT_SECRET appears to contain common weak patterns, consider using a stronger secret".to_string());
            }
        } else {
            validation.errors.push("JWT_SECRET is required for authentication".to_string());
            validation.is_valid = false;
        }
        ...
        if let Ok(session_expires) = std::env::var("SESSION_EXPIRES_IN") {
            ...
            if !session_expires.ends_with('d') && !session_expires.ends_with('h') && 
               !session_expires.ends_with('m') && !session_expires.ends_with('s') {
        ...
        if let Ok(sensitivity_str) = std::env::var("DEFAULT_SENSITIVITY_LEVEL") {
            match sensitivity_str.parse::<u32>() {
                ...
                Err(_) => {
                    validation.errors.push("DEFAULT_SENSITIVITY_LEVEL must be a valid integer".to_string());
                    validation.is_valid = false;
                }
            }
        }
    ...
    fn validate_sync_configuration(&self, validation: &mut StartupValidation) {
        ...
        if let Ok(sync_enabled) = std::env::var("SYNC_ENABLED") {
            match sync_enabled.to_lowercase().as_str() {
                "true" | "false" => {},
                _ => {
                    validation.errors.push("SYNC_ENABLED must be 'true' or 'false'".to_string());
                    validation.is_valid = false;
                }
            }
        }
        ...
        if let Ok(timer_str) = std::env::var("SYNC_TIMER_MS") {
            match timer_str.parse::<u64>() {
                Ok(timer) => {
                    if timer < 1000 {
                        validation.warnings.push("SYNC_TIMER_MS is less than 1000ms, this may cause high CPU usage".to_string());
                    }
                    if timer > 3600000 {
                        validation.warnings.push("SYNC_TIMER_MS is greater than 1 hour, sync may be too infrequent".to_string());
                    }
                },
                Err(_) => {
                    validation.errors.push("SYNC_TIMER_MS must be a valid integer (milliseconds)".to_string());
                    validation.is_valid = false;
                }
            }
        }
        ...
        if let Ok(batch_enabled) = std::env::var("BATCH_SYNC_ENABLED") {
            match batch_enabled.to_lowercase().as_str() {
                "true" | "false" => {},
                _ => {
                    validation.errors.push("BATCH_SYNC_ENABLED must be 'true' or 'false'".to_string());
                    validation.is_valid = false;
                }
            }
            ...
            if batch_enabled.to_lowercase() == "true" {
                ...
                if let Ok(batch_type) = std::env::var("BATCH_SYNC_TYPE") {
                    match batch_type.as_str() {
                        ...
                        _ => {
                            validation.errors.push("BATCH_SYNC_TYPE must be 'round-robin' or 'weighted-round-robin'".to_string());
                            validation.is_valid = false;
                        }
                    }
                }
                ...
                if let Ok(size_str) = std::env::var("BATCH_SYNC_SIZE") {
                    match size_str.parse::<u32>() {
                        Ok(size) => {
                            if size == 0 {
                                validation.errors.push("BATCH_SYNC_SIZE cannot be 0".to_string());
                                validation.is_valid = false;
                            }
                            if size > 10000 {
                                validation.warnings.push("BATCH_SYNC_SIZE is very large, this may cause memory issues".to_string());
                            }
                        },
                        Err(_) => {
                            validation.errors.push("BATCH_SYNC_SIZE must be a valid integer".to_string());
                            validation.is_valid = false;
                        }
                    }
                }
    ...
    fn validate_organization_configuration(&self, validation: &mut StartupValidation) {
        ...
        if let Ok(org_id) = std::env::var("DEFAULT_ORGANIZATION_ID") {
            if org_id.len() != 26 {
                validation.warnings.push("DEFAULT_ORGANIZATION_ID should be 26 characters long (ULID format)".to_string());
            }
        ...
        if let Ok(org_name) = std::env::var("DEFAULT_ORGANIZATION_NAME") {
            if org_name.is_empty() {
                validation.errors.push("DEFAULT_ORGANIZATION_NAME cannot be empty".to_string());
                validation.is_valid = false;
            }
            
            if org_name.len() > 100 {
                validation.warnings.push("DEFAULT_ORGANIZATION_NAME is very long, consider shortening it".to_string());
            }
        }
        ...
        if let Ok(admin_password) = std::env::var("DEFAULT_ORGANIZATION_ADMIN_PASSWORD") {
            if admin_password.len() < 8 {
                validation.errors.push("DEFAULT_ORGANIZATION_ADMIN_PASSWORD must be at least 8 characters long".to_string());
                validation.is_valid = false;
            }
            
            // Check for common weak passwords
            let weak_passwords = vec!["password", "admin", "123456", "qwerty"];
            if weak_passwords.iter().any(|&weak| admin_password.to_lowercase().contains(weak)) {
        ...
        if let Ok(group_id) = std::env::var("GROUP_ID") {
            if group_id.len() != 26 {
                validation.warnings.push("GROUP_ID should be 26 characters long (ULID format)".to_string());
            }
        }
    }
    
    /// Initialize core application services
    async fn initialize_core_services(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        info!("[STARTUP] Cache initialized with type: {:?}, TTL: {:?}", cache_type, ttl);
    ...
    async fn setup_database_and_storage(&self) -> Result<(crate::database::db::AsyncDbPool, aws_sdk_s3::Client, String), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let (s3_client, bucket_name) = storage::initialize().await
            .map_err(|e| {
                error!("[STARTUP] Failed to initialize S3 storage: {}", e);
                e
            })?;
        
        info!("[STARTUP] S3 storage initialized with bucket: {}", bucket_name);
        
        Ok((pool, s3_client, bucket_name))
    }
    
    /// Initialize background services
    async fn initialize_background_services(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn setup_messaging_services(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        SENDER.set(arc_sender).map_err(|_| "Failed to initialize message sender")?;
