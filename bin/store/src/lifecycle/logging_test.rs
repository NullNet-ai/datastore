...
mod tests {
    ...
    async fn should_generate_unique_correlation_ids() {
        ...
        assert!(id1.starts_with("lc-"), "Correlation ID should start with 'lc-'");
        assert!(id2.starts_with("lc-"), "Correlation ID should start with 'lc-'");
        assert!(id3.starts_with("lc-"), "Correlation ID should start with 'lc-'");
        ...
        assert_eq!(id1_parts.len(), 3, "Correlation ID should have 3 parts separated by '-'");
        assert_eq!(id2_parts.len(), 3, "Correlation ID should have 3 parts separated by '-'");
        assert_eq!(id3_parts.len(), 3, "Correlation ID should have 3 parts separated by '-'");
        
        // Verify timestamp and counter are hexadecimal
        assert!(id1_parts[1].chars().all(|c| c.is_ascii_hexdigit()), "Timestamp should be hexadecimal");
        assert!(id1_parts[2].chars().all(|c| c.is_ascii_hexdigit()), "Counter should be hexadecimal");
    ...
    async fn should_automatically_assign_correlation_ids() {
        ...
        for (i, entry) in entries.iter().enumerate() {
            println!("Entry {}: correlation_id = {:?}", i + 1, entry.correlation_id);
            assert!(entry.correlation_id.is_some(), "Entry should have a correlation ID");
            
            let correlation_id = entry.correlation_id.as_ref().unwrap();
            assert!(correlation_id.starts_with("lc-"), "Correlation ID should start with 'lc-'");
            
            // Verify log entry structure
            assert_eq!(entry.component, "TestComponent", "Component should match");
            assert!(entry.message.starts_with("Test message"), "Message should start with 'Test message'");
        ...
        assert_eq!(entries[1].level, LogLevel::Info, "First entry should be Info level");
        assert_eq!(entries[1].category, LogCategory::Lifecycle, "First entry should be Lifecycle category");
        assert_eq!(entries[0].level, LogLevel::Warn, "Second entry should be Warn level");
        assert_eq!(entries[0].category, LogCategory::Runtime, "Second entry should be Runtime category");
        ...
        let id1_parts: Vec<&str> = id1.split('-').collect();
        let id2_parts: Vec<&str> = id2.split('-').collect();
        assert_eq!(id1_parts.len(), 3, "Correlation ID should have proper format");
        assert_eq!(id2_parts.len(), 3, "Correlation ID should have proper format");
    ...
    async fn should_use_provided_correlation_id() {
        ...
        assert_eq!(entry.category, LogCategory::Database, "Log category should be Database");
        assert_eq!(entry.component, "TestComponent", "Component should match");
        assert_eq!(entry.message, "Test message with custom correlation ID", "Message should match");
        ...
        assert!(entry.metadata.is_empty(), "Metadata should be empty by default");
    ...
    async fn should_log_without_correlation_id_when_requested() {
        ...
        logger.log_without_correlation(
            LogLevel::Debug,
        ...
        assert!(entry.correlation_id.is_none(), "Entry should not have a correlation ID");
        
        // Verify all other log entry properties are still valid
        assert_eq!(entry.level, LogLevel::Debug, "Log level should be Debug");
        assert_eq!(entry.category, LogCategory::Performance, "Log category should be Performance");
        assert_eq!(entry.component, "TestComponent", "Component should match");
        assert_eq!(entry.message, "Test message without correlation ID", "Message should match");
