...
mod tests {
    ...
    use tokio;
    use crate::lifecycle::runtime::{RuntimeManager, check_cache_health};
    ...
    async fn should_perform_database_health_check_successfully() {
        ...
        match health_result {
            Ok(()) => {
                ...
                assert!(true, "Database health check should return Ok(()) on success");
            },
            Err(e) => {
                println!("  ⚠️  Database health check failed: {}", e);
                println!("  ℹ️  This may be expected if PostgreSQL is not running on localhost:5433");
                // Verify that we get a proper error message
                let error_msg = e.to_string();
                assert!(!error_msg.is_empty(), "Error message should not be empty");
                assert!(error_msg.contains("database") || error_msg.contains("connection") || 
                       error_msg.contains("timed out") || error_msg.contains("timeout") ||
                       error_msg.contains("Failed to create") || error_msg.contains("refused"), 
                       "Error should be database-related: {}", error_msg);
            }
        }
    ...
    async fn should_perform_cache_health_check_successfully() {
        ...
        use crate::providers::storage::cache::{CacheConfig, cache_factory::CacheType};
        ...
        match cache_result {
            Ok(message) => {
                ...
                assert!(message.contains("healthy") || message.contains("OK") || message.contains("success") || message.contains("verified"), 
                       "Success message should indicate health status: {}", message);
            },
            Err(e) => {
                ...
                assert!(e.to_string().contains("cache") || e.to_string().contains("memory"), 
                       "Error should be cache-related: {}", e);
            }
        }
    ...
    async fn should_handle_cache_health_check_timeout_gracefully() {
        ...
        match timeout_result {
            Ok(message) => {
                ...
                assert!(!message.is_empty(), "Fallback success message should not be empty");
                assert!(message.contains("healthy") || message.contains("OK") || message.contains("success") || message.contains("fallback") || message.contains("verified"), 
                       "Success message should indicate health or fallback status: {}", message);
            },
            Err(e) => {
                ...
                let error_msg = e.to_string();
                assert!(!error_msg.is_empty(), "Error message should not be empty");
                assert!(error_msg.contains("timed out") || error_msg.contains("failed") || 
                       error_msg.contains("connection") || error_msg.contains("unreachable"), 
                       "Error should indicate timeout or connection failure: {}", error_msg);
            }
        }
    ...
    async fn should_timeout_on_unreachable_database_within_reasonable_time() {
        ...
        match RuntimeManager::check_database_health().await {
            ...
            Err(e) => {
                ...
                assert!(elapsed >= Duration::from_millis(1), "Health check should take some time: {:?}", elapsed);
                ...
                assert!(
                    ...
                    error_msg.contains("connection") || error_msg.contains("Failed to create") ||
                    error_msg.contains("refused") || error_msg.contains("unreachable"),
                    "Error message should indicate timeout or connection issue: {}", error_msg
                );
            }
        }
    ...
    async fn should_initialize_runtime_manager_successfully() {
        ...
        assert!(std::mem::size_of_val(&runtime_manager) > 0, "RuntimeManager should have a valid size");
    ...
    async fn should_handle_state_transitions_correctly() {
        ...
        assert!(std::mem::size_of_val(&manager1) == std::mem::size_of_val(&manager2), 
        ...
        assert!(manager_size > 0, "RuntimeManager should have a valid size: {}", manager_size);
