...
install-rust:
	...
	@if [ "$$(uname -s | cut -c1-10)" = "MINGW32_NT" ] || [ "$$(uname -s | cut -c1-10)" = "MINGW64_NT" ] || [ "$$(uname -s | cut -c1-6)" = "CYGWIN" ] || powershell -Command "exit 0" 2>/dev/null; then \
		powershell -Command "if (!(Get-Command rustc -ErrorAction SilentlyContinue)) { Write-Host '🦀 Installing Rust 1.88.0...'; Invoke-WebRequest -Uri 'https://win.rustup.rs/' -OutFile 'rustup-init.exe'; .\rustup-init.exe -y --default-toolchain 1.88.0; Remove-Item rustup-init.exe } else { Write-Host '🔍 Checking Rust version...'; $$rustVersion = (rustc --version).Split(' ')[1]; if ($$rustVersion -ne '1.88.0') { Write-Host '⚠️  Current Rust version: ' + $$rustVersion + ', required: 1.88.0'; Write-Host '🔄 Installing Rust 1.88.0...'; rustup install 1.88.0; rustup default 1.88.0 } else { Write-Host '✅ Rust 1.88.0 already installed' } }"; \
	else \
		if ! command -v rustc >/dev/null 2>&1; then \
			...
			if [ "$$RUST_VERSION" != "1.88.0" ]; then \
				echo "⚠️  Current Rust version: $$RUST_VERSION, required: 1.88.0"; \
				echo "🔄 Installing Rust 1.88.0..."; \
...
install:
	...
	@if [ "$$(uname)" = "Darwin" ]; then \
		...
		echo "  - Rust 1.88.0 (https://rustup.rs/)"; \
		echo "  - PostgreSQL"; \
